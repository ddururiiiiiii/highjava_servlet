...
public class WebController extends HttpServlet{
   ...
   //Logger 객체 생성 - sysout 대신에 찍어보기 위해 생성함.
   private static Logger LOGGER = Logger.getLogger(WebController.class);
   
   //매핑정보 저장 (핸들러 객체 저장용 맵)
   private Map<String, CommandHandler> cmmHandlerMap = new HashMap<>(); //핸드
   ...
   private void process(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
      String reqURI = req.getRequestURI(); //url를 Request객체로 부터 꺼내오기
      
      if(reqURI.indexOf(req.getContextPath()) == 0) {
         //ContextPath 부분을 제외한 URL 정보 가져오기
         reqURI = reqURI.substring(req.getContextPath().length()); //member/list.jsp
      }
      
     /* if(reqURI.equals("/member/list.do")) { //회원목록 조회
         //회원목록 조회기능 호출...
      }else if(reqURI.equals("/member/insert.do")) { //회원등록
         if(req.getMethod().equals("GET")) { //GET방식이면
            //등록을 위한 폼페이지 이동
      ...
      CommandHandler handler = cmmHandlerMap.get(reqURI); //적절한 핸들러를 가져올 수 있다. if-else를 쓰지않고, 수정할 필요도 없다.!! 
